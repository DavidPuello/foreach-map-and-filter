...
function doubleValuesWithMap(arr) {
    let doubleArray = arr.map(function(value) {
        return value*2;
    });
...
function extractKey(arr, key){
    let newArray = arr.map(function(obj) {
        return obj[key];
    });
...
function filterByValue(arr, key) {
    let filterArray = arr.filter(function(obj) {
...
function findInObj(arr, key, searchValue) {
    let foundObject = arr.find(function(obj) {
...
function removeVowels(str) {
    ...
    let newStr = str.toLowerCase().replace(/[aeiou]/gi, '');
...
function doubleOddNumbers(arr) {
    let doubleOddNumbers = arr.filter(function(number) {
        return number % 2 !== 0;
